...
public class SwipeMenuView extends LinearLayout {
    ...
    private void addItem(SwipeMenuItem item, int index) {
        //LayoutParams params = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, item.getHeight());
        LayoutParams params = new LayoutParams(item.getWidth(), item.getHeight());
        params.weight = item.getWeight();
        LinearLayout parent = new LinearLayout(getContext());
        parent.setId(index);
        parent.setGravity(Gravity.LEFT | Gravity.CENTER_VERTICAL);
    ...
    private ImageView createIcon(SwipeMenuItem item) {
        ...
        params.leftMargin = item.getWidth() / 12 - imageView.getWidth() / 2 - getPaddingLeft();
    ...
    public RecyclerView.ViewHolder getAdaterViewHolder() {
        return mAdapterVIewHolder;
    }
